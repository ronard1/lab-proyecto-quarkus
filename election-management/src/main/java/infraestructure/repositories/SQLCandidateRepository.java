...
public class SQLCandidateRepository implements CandidateRepository {
    ...
    public void save(List<Candidate> candidates) {
        candidates.stream()
                .map(infraestructure.repositories.entities.Candidate::fromDomain)
    ...
    public List<Candidate> find(CandidateQuery query) {
        ...
        var cq = cb.createQuery(infraestructure.repositories.entities.Candidate.class);
        var root = cq.from(infraestructure.repositories.entities.Candidate.class);

        cq.select(root).where(conditions(query, cb, root));

        return entityManager.createQuery(cq)
                .getResultStream()
                .map(infraestructure.repositories.entities.Candidate::toDomain)
                .toList();
    }

    private Predicate[] conditions(CandidateQuery query,
                                   CriteriaBuilder cb,
                                   Root<infraestructure.repositories.entities.Candidate> root) {
